...
func (q *Queries) AccountByEmail(ctx context.Context, email sql.NullString) (Account, error) {
...
const accountByPhone = `-- name: AccountByPhone :one
SELECT id, create_at, update_at, user_id, region, username, area, phone, email, avatar, password, pwd_level, platform, state
FROM accounts
WHERE phone = $1
`

func (q *Queries) AccountByPhone(ctx context.Context, phone sql.NullString) (Account, error) {
	row := q.db.QueryRowContext(ctx, accountByPhone, phone)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.CreateAt,
