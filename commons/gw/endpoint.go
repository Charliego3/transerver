...
const (
	// CredentialRequire - Credentials/certificate required for thi type of connection.
	CredentialRequire CredsRequirement = iota
	// CredentialDrop - Credentials/certificate not needed and should get ignored.
	CredentialDrop
	// CredentialOptional - Credentials/certificate might be used if supplied
	CredentialOptional
)
...
func schemeToCredentialRequirement(schema string) CredsRequirement {
	switch schema {
	case "https", "unixs":
		return CredentialRequire
	case "http":
		return CredentialDrop
	case "unix":
...
func translateEndpoint(ep string) (addr string, serverName string, requireCreds CredsRequirement) {
	if strings.HasPrefix(ep, "unix:") || strings.HasPrefix(ep, "unixs:") {
		if strings.HasPrefix(ep, "unix:///") || strings.HasPrefix(ep, "unixs:///") {
			...
			return "unix://" + absolutePath, extractHostFromPath(absolutePath), schemeToCredentialRequirement(schema)
		}
		if strings.HasPrefix(ep, "unix://") || strings.HasPrefix(ep, "unixs://") {
			// legacy etcd local path
			schema, localPath := mustSplit2(ep, "://")
			return "unix:" + localPath, extractHostFromPath(localPath), schemeToCredentialRequirement(schema)
		}
		schema, localPath := mustSplit2(ep, ":")
		return "unix:" + localPath, extractHostFromPath(localPath), schemeToCredentialRequirement(schema)
	}
	...
	if strings.Contains(ep, "://") {
		u, err := url.Parse(ep)
		if err != nil {
			return ep, extractHostFromHostPort(ep), CredentialOptional
		}
		if u.Scheme == "http" || u.Scheme == "https" {
			return u.Host, u.Hostname(), schemeToCredentialRequirement(u.Scheme)
		}
		return ep, u.Hostname(), schemeToCredentialRequirement(u.Scheme)
	}
