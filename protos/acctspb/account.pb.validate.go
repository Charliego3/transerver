...
func (m *RegisterRequest) validate(all bool) error {
	...
	if m.GetUname() != "" {
		...
		if l := utf8.RuneCountInString(m.GetUname()); l < 8 || l > 32 {
			err := RegisterRequestValidationError{
				field:  "Uname",
...
var _RegisterRequest_Password_Pattern = regexp.MustCompile("\\S+")
...
func (m *LoginRequest) validate(all bool) error {
	...
	if !_LoginRequest_Unique_Pattern.MatchString(m.GetUnique()) {
	...
	if !_LoginRequest_Region_Pattern.MatchString(m.GetRegion()) {
		err := LoginRequestValidationError{
			field:  "Region",
			reason: "value does not match regex pattern \"[A-Z]{2,}\"",
		}
	...
	if !_LoginRequest_Uname_Pattern.MatchString(m.GetUname()) {
		err := LoginRequestValidationError{
			field:  "Uname",
			reason: "value does not match regex pattern \"\\\\S+\"",
		}
...
var _LoginRequest_Unique_Pattern = regexp.MustCompile("[A-Za-z0-9_-]+")

var _LoginRequest_Region_Pattern = regexp.MustCompile("[A-Z]{2,}")

var _LoginRequest_Uname_Pattern = regexp.MustCompile("\\S+")

var _LoginRequest_Password_Pattern = regexp.MustCompile("\\S+")
